...
const { apikey, bot_token, owner, ownerLink, version, prefix } = JSON.parse(
...
bot.on("left_chat_member", async (lol) => {
  ...
  await lol.replyWithPhoto({
    url: `https://api.lolhuman.xyz/api/base/leave?apikey=${apikey}&img1=${pp_user}&img2=${pp_group}&background=https://i.ibb.co/8B6Q84n/LTqHsfYS.jpg&username=${full_name}&member=${groupmembers}&groupname=${groupname}`,
  });
});
...
bot.on("callback_query", async (lol) => {
  ...
  if (lol.callbackQuery.from.id != user_id)
    return lol.answerCbQuery(
      "Sorry, You do not have the right to access this button.",
      { show_alert: true }
    );
  ...
  if (!isGroup)
    console.log(
      chalk.whiteBright("├"),
      chalk.cyanBright("[ ACTIONS ]"),
      chalk.whiteBright(callback_data),
  ...
  if (callback_data == "help")
    return await help[callback_data](lol, user.full_name, user_id);
  ...
  try {
    ...
    const reply = async (text) => {
      for (var x of range(0, text.length, 4096)) {
        return await lol.replyWithMarkdown(text.substr(x, 4096), {
    ...
    if (
      entertainment[lol.update.message.from.id] &&
      entertainment[lol.update.message.from.id] ===
        lol.update.message.text.toLowerCase()
    ) {
    ...
    if (isQuoted) {
      file_id = isQuotedImage
        ? lol.message.reply_to_message.photo[
            lol.message.reply_to_message.photo.length - 1
          ].file_id
        : isQuotedVideo
    ...
    switch (command) {
      ...
      case "listsurah":
        result = await fetchJson(
          `https://api.lolhuman.xyz/api/quran?apikey=${apikey}`
        );
      ...
      case "alquranaudio":
        ...
        surah = args[0];
        await lol.replyWithAudio({
          url: `https://api.lolhuman.xyz/api/quran/audio/${surah}?apikey=${apikey}`,
        });
        break;
      case "asmaulhusna":
        result = await fetchJson(
          `https://api.lolhuman.xyz/api/asmaulhusna?apikey=${apikey}`
        );
        result = result.result;
      ...
      case "kisahnabi":
        if (args.length == 0)
          return await reply(`Example: ${prefix + command} Muhammad`);
        query = args.join(" ");
        result = await fetchJson(
          `https://api.lolhuman.xyz/api/kisahnabi/${query}?apikey=${apikey}`
        );
        result = result.result;
      ...
      case "jadwalsholat":
        if (args.length == 0)
          return await reply(`Example: ${prefix + command} Yogyakarta`);
        daerah = args.join(" ");
        result = await fetchJson(
          `https://api.lolhuman.xyz/api/sholat/${daerah}?apikey=${apikey}`
        );
        result = result.result;
      ...
      case "ytplay":
        ...
        await fetchJson(
          `https://api.lolhuman.xyz/api/ytsearch?apikey=${apikey}&query=${args.join(
            " "
          )}`
        ).then(async (result) => {
          await fetchJson(
            `https://api.lolhuman.xyz/api/ytaudio2?apikey=${apikey}&url=https://www.youtube.com/watch?v=${result.result[0].videoId}`
          ).then(async (result) => {
            await lol.replyWithAudio(
              { url: result.result.link, filename: result.result.title },
      ...
      case "ytsearch":
        ...
        try {
          query = args.join(" ");
          result = await fetchJson(
            `https://api.lolhuman.xyz/api/ytsearch?apikey=${apikey}&query=${query}`
          );
      ...
      case "ytmp3":
        ...
        if (Number(result.size.split(` MB`)[0]) >= 50.0)
          return await reply(`Sorry the bot cannot send more than 50 MB!`);
      ...
      case "ig":
        if (args.length == 0)
          return reply(
            `Example: ${
              prefix + command
            } https://www.instagram.com/p/CU0MhPjBZO2/`
          );
        return fetchJson(
      ...
      case "twitterimage":
        if (args.length == 0)
          return await reply(
            `Example: ${
              prefix + command
            } https://twitter.com/memefess/status/1385161473232543747`
          );
      ...
      case "spotify":
        ...
        await lol.replyWithAudio(
          { url: result.link },
          { title: result.title, thumb: result.thumbnail }
        );
        break;
      case "spotifysearch":
        if (args.length == 0)
        ...
        try {
          query = args.join(" ");
          result = await fetchJson(
            `https://api.lolhuman.xyz/api/spotifysearch?apikey=${apikey}&query=${query}`
          );
          hasil = result.result.slice(0, 3);
          hasil.forEach(async (res) => {
            caption = `\`❖ Title     :\` *${res.title}*\n`;
      ...
      case "jooxplay":
        ...
        query = args.join(" ");
        result = await fetchJson(
          `https://api.lolhuman.xyz/api/jooxplay?apikey=${apikey}&query=${query}`
        );
        result = result.result;
        ...
        if ((caption + result.lirik).length >= 1024) {
          ...
          await lol.replyWithPhoto(
            { url: result.image },
            { caption: caption + result.lirik, parse_mode: "Markdown" }
          );
        }
        await lol.replyWithAudio(
          { url: result.audio[0].link, filename: result.info.song },
          { thumb: result.image }
        );
        break;
      case "zippyshare":
      ...
      case "pinterest":
        if (args.length == 0)
          return await reply(`Example: ${prefix + command} loli kawaii`);
        query = args.join(" ");
        url = await fetchJson(
          `https://api.lolhuman.xyz/api/pinterest?apikey=${apikey}&query=${query}`
        );
        url = url.result;
      ...
      case "pixiv":
        ...
        query = args.join(" ");
        await lol.replyWithPhoto({
          url: `https://api.lolhuman.xyz/api/pixiv?apikey=${apikey}&query=${query}`,
        });
        break;
      case "pixivdl":
        if (args.length == 0)
          return await reply(`Example: ${prefix + command} 63456028`);
        pixivid = args[0];
        await lol.replyWithPhoto({
          url: `https://api.lolhuman.xyz/api/pixivdl/${pixivid}?apikey=${apikey}`,
        });
        break;
      ...
      case "caklontong":
        var { result } = await fetchJson(
          `https://api.lolhuman.xyz/api/tebak/caklontong2?apikey=${apikey}`
        );
      ...
      case "siapaaku":
        var { result } = await fetchJson(
          `https://api.lolhuman.xyz/api/tebak/siapaaku?apikey=${apikey}`
        );
        entertainment[lol.update.message.from.id] = result.answer.toLowerCase();
        lol.reply(result.question);
        break;
      case "tebakgambar":
        var { result } = await fetchJson(
          `https://api.lolhuman.xyz/api/tebak/gambar2?apikey=${apikey}`
        );
        entertainment[lol.update.message.from.id] = result.answer.toLowerCase();
      ...
      case "tebakkata":
        var { result } = await fetchJson(
          `https://api.lolhuman.xyz/api/tebak/kata?apikey=${apikey}`
        );
        entertainment[lol.update.message.from.id] =
      ...
      case "tebakjenaka":
        var { result } = await fetchJson(
          `https://api.lolhuman.xyz/api/tebak/jenaka?apikey=${apikey}`
        );
        entertainment[lol.update.message.from.id] = result.answer.toLowerCase();
        lol.reply(result.question);
        break;
      ...
      case "reverse":
        if (!isQuotedImage)
          return await reply(`Please reply a image use this command.`);
        google = await fetchJson(
          `https://api.lolhuman.xyz/api/googlereverse?apikey=${apikey}&img=${mediaLink}`
        );
        yandex = await fetchJson(
          `https://api.lolhuman.xyz/api/reverseyandex?apikey=${apikey}&img=${mediaLink}`
        );
        options = {
          reply_markup: {
            inline_keyboard: [
      ...
      case "character":
        if (args.length == 0)
          return await reply(`Example: ${prefix + command} Miku Nakano`);
        query = args.join(" ");
        result = await fetchJson(
          `https://api.lolhuman.xyz/api/character?apikey=${apikey}&query=${query}`
        );
        result = result.result;
        text = `Id : ${result.id}\n`;
      ...
      case "manga":
        ...
        query = args.join(" ");
        result = await fetchJson(
          `https://api.lolhuman.xyz/api/manga?apikey=${apikey}&query=${query}`
        );
        result = result.result;
        text = `Id : ${result.id}\n`;
      ...
      case "anime":
        ...
        query = args.join(" ");
        result = await fetchJson(
          `https://api.lolhuman.xyz/api/anime?apikey=${apikey}&query=${query}`
        );
        result = result.result;
        text = `Id : ${result.id}\n`;
        text += `Id MAL : ${result.idMal}\n`;
      ...
      case "wait":
        if (
          ...
          result = await fetchJson(
            `https://api.lolhuman.xyz/api/wait?apikey=${apikey}&img=${url_file}`
          );
          result = result.result;
      ...
      case "kusonime":
        ...
        if (text.length <= 1024) {
          return await lol.replyWithPhoto(
            { url: result.thumbnail },
            { caption: text }
          );
        }
      ...
      case "kusonimesearch":
        ...
        query = args.join(" ");
        result = await fetchJson(
          `https://api.lolhuman.xyz/api/kusonimesearch?apikey=${apikey}&query=${query}`
        );
        result = result.result;
        text = `*Title : ${result.title}\n`;
        text += `**Japanese : ${result.japanese}\n`;
      ...
      case "otakudesusearch":
        ...
        query = args.join(" ");
        result = await fetchJson(
          `https://api.lolhuman.xyz/api/otakudesusearch?apikey=${apikey}&query=${query}`
        );
        result = result.result;
        text = `Title : ${result.title}\n`;
        text += `Japanese : ${result.japanese}\n`;
      ...
      case "lk21":
        if (args.length == 0)
          return await reply(`Example: ${prefix + command} Transformer`);
        query = args.join(" ");
        result = await fetchJson(
          `https://api.lolhuman.xyz/api/lk21?apikey=${apikey}&query=${query}`
        );
        result = result.result;
        text = `Title : ${result.title}\n`;
        ...
        break;
      case "drakorongoing":
        // Fungsi untuk mengambil data drakor ongoing
        async function getOngoingDrakor(apidrakor) {
          const apiUrl = `https://api.lolhuman.xyz/api/drakorongoing?apikey=${apidrakor}`;
          const response = await fetch(apiUrl);
          const data = await response.json();
          return data.result;
        }

        const apidrakor = "380de1f174fe407bdea39dac"; // Ganti dengan kunci API Anda
        const maxDramaToShow = 5; // Jumlah drama yang akan ditampilkan
        const ongoingDrakor = await getOngoingDrakor(apidrakor);

        for (
          let i = 0;
          i < Math.min(maxDramaToShow, ongoingDrakor.length);
          i++
        ) {
          const drakor = ongoingDrakor[i];
          let responseText = `${drakor.title}\n`;
          responseText += `Episode: ${drakor.total_episode}\n`;
          responseText += `Genre: ${drakor.genre.join(", ")}\n`;
          responseText += `Link: ${drakor.link}\n`;

          // Mengirim gambar thumbnail
          const thumbnailUrl = drakor.thumbnail;
          await lol.replyWithPhoto(
            { url: thumbnailUrl },
            { caption: responseText }
          );

          // Memberikan jeda sebentar antara setiap pesan
      ...
      case "wattpadsearch":
        if (args.length == 0)
          return await reply(`Example: ${prefix + command} Tere Liye`);
        query = args.join(" ");
        result = await fetchJson(
          `https://api.lolhuman.xyz/api/wattpadsearch?apikey=${apikey}&query=${query}`
        );
        result = result.result;
        text = "Wattpad Seach : \n";
        for (var x of result) {
      ...
      case "thunder":
        if (args.length == 0)
          return await reply(`Example: ${prefix + command} LoL Human`);
        text = args.join(" ");
        await lol.replyWithPhoto({
          url: `https://api.lolhuman.xyz/api/textprome/${command}?apikey=${apikey}&text=${text}`,
        });
        break;
      case "pornhub":
      case "glitch":
      ...
      case "carvedwood":
        if (args.length == 0)
          return await reply(`Example: ${prefix + command} LoL Human`);
        await lol.replyWithPhoto({
          url: `https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${apikey}&text=${args.join(
            " "
          )}`,
        });
        break;
      ...
      case "avatardota":
        if (args.length == 0)
          return await reply(`Example: ${prefix + command} LoL Human`);
        await lol.replyWithPhoto({
          url: `https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey}&text=${args.join(
            " "
          )}`,
        });
        break;
      ...
      case "stalkig":
        if (args.length === 0) {
          return await reply(
            "Masukkan nama pengguna Instagram yang ingin Anda stalk."
          );
        }
        const username = args[0];
        const api = "380de1f174fe407bdea39dac";
        const stalkUrl = `https://api.lolhuman.xyz/api/stalkig/${username}?apikey=${api}`;

        try {
          const response = await fetch(stalkUrl);
          const data = await response.json();

          if (data.status === 200) {
            let responseText = `*Informasi Stalking Instagram *${data.result.username}:\n`;
            responseText += `Nama: ${data.result.fullname}\n`;
            responseText += `Username: ${data.result.username}\n`;
            responseText += `Jumlah Followers: ${data.result.followers}\n`;
            responseText += `Jumlah Following: ${data.result.following}\n`;
            responseText += `Jumlah Postingan: ${data.result.posts}\n`;
            responseText += `Bio: ${data.result.bio}\n`;
            responseText += `Link: ${data.result.link}`;

            // Mengirim foto profil sebagai gambar
            await lol.replyWithPhoto(
            ...
            await reply(
              "Tidak dapat menemukan informasi akun Instagram tersebut."
            );
          }
        } catch (error) {
      ...
      case "stalktiktok":
        if (args.length === 0) {
          return await reply("Masukkan username TikTok yang ingin Anda stalk.");
        }
        const tiktokUsername = args[0];
        const tiktokApi = "380de1f174fe407bdea39dac";
        const tiktokStalkUrl = `https://api.lolhuman.xyz/api/stalktiktok/${tiktokUsername}?apikey=${tiktokApi}`;

        try {
          const response = await fetch(tiktokStalkUrl);
          const data = await response.json();

          if (data.status === 200) {
            let responseText = `*Informasi Stalking TikTok* ${data.result.username}:\n`;
            responseText += `Nama Pengguna: ${data.result.username}\n`;
            responseText += `Nama Tampilan: ${data.result.nickname}\n`;
            responseText += `Jumlah Followers: ${data.result.followers}\n`;
            responseText += `Jumlah Following: ${data.result.followings}\n`;
            responseText += `Jumlah Likes: ${data.result.likes}\n`;
            responseText += `Jumlah Video: ${data.result.video}\n`;
            responseText += `Bio: ${data.result.bio}\n`;
            // Mengirim foto profil sebagai gambar
            await lol.replyWithPhoto(
            ...
            await reply(
              "Tidak dapat menemukan informasi akun TikTok tersebut."
            );
          }
        } catch (error) {
          await reply("Terjadi kesalahan saat memproses permintaan.");
      ...
      case "ytchanel":
        if (args.length === 0) {
          return await reply(
            "Masukkan nama saluran YouTube yang ingin Anda cari."
          );
        }
        const query = args.join(" ");
        const apikeyyt = "380de1f174fe407bdea39dac"; // Ganti dengan kunci API Anda
        const ytChannelUrl = `https://api.lolhuman.xyz/api/ytchannel?apikey=${apikeyyt}&query=${encodeURIComponent(
          query
        )}`;

        try {
          const response = await fetch(ytChannelUrl);
          const data = await response.json();

          if (data.status === 200 && data.result.length > 0) {
            for (const channelInfo of data.result) {
              let responseText = `Nama Saluran: ${channelInfo.channel_name}\n`;
              responseText += `Deskripsi: ${channelInfo.channel_about}\n`;
              responseText += `Tanggal Bergabung: ${channelInfo.channel_created}\n`;
              responseText += `Link: https://www.youtube.com/channel/${channelInfo.channel_id}`;

              // Mengirim foto profil saluran sebagai gambar
              await lol.replyWithPhoto(
                { url: channelInfo.channel_picture.high.url },
                { caption: responseText }
              );
              await new Promise((resolve) => setTimeout(resolve, 1000));
            }
            ...
            await reply(
              "Tidak dapat menemukan informasi saluran YouTube tersebut."
            );
          }
        } catch (error) {
          await reply("Terjadi kesalahan saat memproses permintaan.");
